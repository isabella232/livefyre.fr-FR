---
description: Activez la fonction de commentaires en direct sur votre page.
seo-description: Activez la fonction de commentaires en direct sur votre page.
...
Activez la fonction de commentaires en direct sur votre page. Les commentaires vous permettent de remplacer votre système de commentaires par défaut par des conversations en temps réel sur votre page.
...
| **attachementDelegate** | *objet facultatif* | Permet de spécifier les types de pièces jointes visibles dans le flux d’application. Pour plus d’informations, voir [Restriction des médias](/help/implementation/c-app-customizations/c-restrict-media.md#c_restrict_media). |
...
| **authPageReload** | *booléen facultatif* | S’applique à l’application de commentaires : Permet de contrôler si le commentaire d’un utilisateur est stocké localement pendant le processus d’authentification. Si la valeur est true, si un utilisateur saisit un commentaire, puis se connecte à l’application, le commentaire est stocké localement et est de nouveau saisi dans le champ de contenu après la connexion et l’actualisation de la page. Si la valeur est false, le contenu saisi sera effacé pendant le processus de connexion et doit être tapé à nouveau. |
| **collectionMeta** | *chaîne obligatoire* | Métadonnées codées JWT relatives à la collection. Voir [CollectionMeta](#c_collectionmeta_object) Object pour plus d’informations. |
...
| **datetimeFormat** | *fonction facultative* d’objet string | Indique le format datetime du contenu diffusé en continu. Pour plus d’informations, voir Personnalisation des tampons de date et d’heure. |
...
| **disableThirdPartyAnalytics** | *booléen facultatif* | Désactive les systèmes d’analyse tiers (Quantserve et Google Analytics) que Livefyre peut utiliser pour les mesures internes. La valeur par défaut est false. |
...
| **url** | *chaîne obligatoire* | URL absolue canonique que vous souhaitez joindre à cette collection. Cette URL sera utilisée pour générer des liens vers l’application à partir du contenu partagé sur Facebook et Twitter, des notifications par courrier électronique et Livefyre Studio.  <br>**Note** Livefyre exige l'utilisation d'un nom de domaine complet; le numéro de port ou un rappel pour résoudre la configuration locale n'est pas requis. Si le test est effectué localement, veillez à utiliser un domaine d’URL de base valide. <br>Par exemple : `https://customer.com` est valide, alors que `https://localhost:5995` non. Une fois que vous avez configuré votre serveur Web local pour accepter un nom de domaine complet, aucun rappel ou aucune résolution n’est nécessaire et le développement local peut se poursuivre comme prévu. |
| **type** | *chaîne obligatoire* | Type de collection. Ça doit être `livechat`. |

L’ `CollectionMeta` objet peut également contenir le paramètre facultatif suivant :

| Paramètre | Type | Description |
|---|---|---|
| **balises** | *chaîne facultative* | Liste de mots-clés ou d’expressions uniques séparés par des virgules. Recherchez Collections par balises dans Studio ou avec l’API de recherche. <br> **** Remarque : Bien que les balises ajoutées via Studio puissent contenir des espaces, les balises saisies via l’API ne le peuvent pas. Utilisez des traits de soulignement pour définir des balises qui afficheront des espaces dans l’interface utilisateur. (Par exemple : permet `Monday_Quarterback` d’afficher le quart du lundi dans Studio.) |
